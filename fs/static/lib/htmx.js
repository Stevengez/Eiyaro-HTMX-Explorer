    ...
    return function() {
      ...
      function l(ze) {
        ...
        if (Jr === "head" && (Zr = Zr.replace(S, "")), Q.config.useTemplateFragments && Qr) {
          var Gr = s("<body><template>" + Zr + "</template></body>", 0), Yr = Gr.querySelector("template").content;
          return Q.config.allowScriptTags ? oe(Yr.querySelectorAll("script"), function(Wr) {
            Q.config.inlineScriptNonce && (Wr.nonce = Q.config.inlineScriptNonce), Wr.htmxExecuted = navigator.userAgent.indexOf("Firefox") === -1;
          }) : oe(Yr.querySelectorAll("script"), function(Wr) {
            _(Wr);
          }), Yr;
        }
        switch (Jr) {
      ...
      function Ee(ze, Qr, Jr) {
        ...
        return Yr ? (oe(Yr, function(Wr) {
          var Kr, en = Qr.cloneNode(!0);
          Kr = re().createDocumentFragment(), Kr.appendChild(en), Se(Gr, Wr) || (Kr = en);
          var tn = { shouldSwap: !0, target: Wr, fragment: Kr };
          ce(Wr, "htmx:oobBeforeSwap", tn) && (Wr = tn.target, tn.shouldSwap && Fe(Gr, Wr, Wr, Kr, Jr), oe(Jr.elts, function(rn) {
      ...
      function Ce(ze, Qr, Jr) {
        ...
        if (Zr)
          for (var Gr = Zr.split(","), Yr = 0; Yr < Gr.length; Yr++) {
            var Wr = Gr[Yr].split(":", 2), Kr = Wr[0].trim();
            Kr.indexOf("#") === 0 && (Kr = Kr.substring(1));
            var en = Wr[1] || "true", tn = Qr.querySelector("#" + Kr);
            tn && Ee(en, tn, Jr);
          }
      ...
      function Te(ze, Qr, Jr) {
        oe(Qr.querySelectorAll("[id]"), function(Zr) {
          var Gr = ee(Zr, "id");
          if (Gr && Gr.length > 0) {
            var Yr = Gr.replace("'", "\\'"), Wr = Zr.tagName.replace(":", "\\:"), Kr = ze.querySelector(Wr + "[id='" + Yr + "']");
            if (Kr && Kr !== ze) {
              var en = Zr.cloneNode();
      ...
      function Ie(ze, Qr, Jr) {
        ...
        for (a(u(ze), ze, Qr, Jr), Gr == null ? Zr = u(ze).firstChild : Zr = Gr.nextSibling, Jr.elts = Jr.elts.filter(function(Yr) {
          return Yr != ze;
        }); Zr && Zr !== ze; )
      ...
      function Fe(ze, Qr, Jr, Zr, Gr) {
        switch (ze) {
          ...
          case "delete":
            ...
            return;
          default:
            for (var Yr = Fr(Qr), Wr = 0; Wr < Yr.length; Wr++) {
              var Kr = Yr[Wr];
              try {
      ...
      function je(ze, Qr, Jr, Zr, Gr, Yr) {
        Gr.title = Ve(Zr);
        var Wr = l(Zr);
        if (Wr)
          return Ce(Jr, Wr, Gr), Wr = Be(Jr, Wr, Yr), Re(Wr), Fe(ze, Jr, Qr, Wr, Gr);
      }
      function _e(ze, Qr, Jr) {
        ...
        if (Zr.indexOf("{") === 0) {
          var Gr = E(Zr);
          for (var Yr in Gr)
            if (Gr.hasOwnProperty(Yr)) {
              var Wr = Gr[Yr];
              P(Wr) || (Wr = { value: Wr }), ce(Jr, Yr, Wr);
            }
        } else
      ...
      function et(ze, Qr, Jr) {
        if (Qr[0] === "[") {
          Qr.shift();
          for (var Zr = 1, Gr = " return (function(" + Jr + "){ return (", Yr = null; Qr.length > 0; ) {
            var Wr = Qr[0];
            if (Wr === "]") {
              if (Zr--, Zr === 0) {
                Yr === null && (Gr = Gr + "true"), Qr.shift(), Gr += ")})";
                try {
              ...
              Wr === "[" && Zr++;
            Qe(Wr, Yr, Jr) ? Gr += "((" + Jr + "." + Wr + ") ? (" + Jr + "." + Wr + ") : (window." + Wr + "))" : Gr = Gr + Wr, Yr = Qr.shift();
          }
        }
      }
      function y(ze, Qr) {
      ...
      function nt(ze, Qr, Jr) {
        ...
        do {
          ...
          if (Wr !== "")
            ...
            } else if (Wr.indexOf("sse:") === 0)
              Zr.push({ trigger: "sse", sseEvent: Wr.substr(4) });
            else {
              var tn = { trigger: Wr }, en = et(ze, Gr, "event");
          ...
          Gr.length === Yr && fe(ze, "htmx:syntax:error", { token: Gr.shift() }), y(Gr, Je);
        } while (Gr[0] === "," && Gr.shift());
      ...
      function lt(ze, Qr, Jr) {
        if (ze.tagName === "A" && st(ze) && (ze.target === "" || ze.target === "_self") || ze.tagName === "FORM") {
          ...
          if (ze.tagName === "A")
            ...
            var Yr = ee(ze, "method");
            Zr = Yr ? Yr.toLowerCase() : "get", Gr = ee(ze, "action");
          }
          Jr.forEach(function(Wr) {
      ...
      function ht(ze, Qr, Jr, Zr, Gr) {
        var Yr = ae(ze), Wr;
        Zr.from ? Wr = Z(ze, Zr.from) : Wr = [ze], Zr.changed && Wr.forEach(function(Kr) {
          var en = ae(Kr);
      ...
      function xt(ze, Qr, Jr) {
        if (se(ze)) {
          ...
          Gr.onerror = function(Yr) {
            fe(ze, "htmx:wsError", { error: Yr, socket: Gr }), yt(ze);
          }, Gr.onclose = function(Yr) {
          ...
          }, Gr.onopen = function(Yr) {
            Jr = 0;
          }, ae(ze).webSocket = Gr, Gr.addEventListener("message", function(Yr) {
            if (!yt(ze)) {
              var Wr = Yr.data;
              R(ze, function(on) {
                Wr = on.transformResponse(Wr, null, ze);
              });
              for (var Kr = T(ze), en = l(Wr), tn = M(en.children), rn = 0; rn < tn.length; rn++) {
                var nn = tn[rn];
      ...
      function bt(ze) {
        ...
        Qr ? ze.addEventListener(it(ze)[0].trigger, function(Jr) {
          var Zr = ae(Qr).webSocket, Gr = xr(ze, Qr), Yr = dr(ze, "post"), Wr = Yr.errors, Kr = Yr.values, en = Hr(ze), tn = le(Kr, en), rn = yr(tn, ze);
          if (rn.HEADERS = Gr, Wr && Wr.length > 0) {
            ce(ze, "htmx:validation:halted", Wr);
            return;
          }
      ...
      function Ct(ze, Qr) {
        var Jr = c(ze, Ot);
        if (Jr) {
          var Zr = ae(Jr).sseEventSource, Gr = function(Yr) {
            if (!Tt(Jr)) {
              if (!se(ze)) {
              ...
              var Wr = Yr.data;
              R(ze, function(rn) {
                Wr = rn.transformResponse(Wr, null, ze);
              });
              var Kr = wr(ze), en = ye(ze), tn = T(ze);
              je(Kr.swapStyle, en, ze, Wr, tn), nr(tn.tasks), ce(ze, "htmx:sseMessage", Yr);
            }
          };
      ...
      function Rt(ze, Qr, Jr) {
        var Zr = c(ze, Ot);
        if (Zr) {
          var Gr = ae(Zr).sseEventSource, Yr = function() {
            Tt(Zr) || (se(ze) ? Qr(ze) : Gr.removeEventListener(Jr, Yr));
          };
          ae(ze).sseListener = Yr, Gr.addEventListener(Jr, Yr);
        } else
          fe(ze, "htmx:noSSESourceError");
      }
      function Tt(ze) {
      ...
      function Ht(ze, Qr, Jr) {
        ...
        return oe(w, function(Gr) {
          if (o(ze, "hx-" + Gr)) {
            var Yr = te(ze, "hx-" + Gr);
            Zr = !0, Qr.path = Yr, Qr.verb = Gr, Jr.forEach(function(Wr) {
              Lt(ze, Wr, Qr, function(Kr, en) {
                if (v(Kr, Q.config.disableSelector)) {
                  m(Kr);
                  return;
                }
      ...
      function Lt(ze, Qr, Jr, Zr) {
        if (Qr.sseEvent)
          ...
          var Yr = new IntersectionObserver(function(Wr) {
      ...
      function kt(ze) {
        ...
        if (It(ze) && Jr.push(ze), document.evaluate)
          ...
          for (var Gr = ze.getElementsByTagName("*"), Yr = 0; Yr < Gr.length; Yr++)
            It(Gr[Yr]) && Jr.push(Gr[Yr]);
        return Jr;
      }
      function Pt(ze) {
      ...
      function Ft(ze, Qr, Jr) {
        ...
        var Gr, Yr = function(Wr) {
          return Tr(ze, function() {
            Gr || (Gr = new Function("event", Jr)), Gr.call(ze, Wr);
          });
        };
...
`), Gr = null, Yr = 0; Zr.length > 0; ) {
            var Wr = Zr.shift(), Kr = Wr.match(/^\s*([a-zA-Z:\-\.]+:)(.*)/);
            Yr === 0 && Kr ? (Wr.split(":"), Gr = Kr[1].slice(0, -1), Jr[Gr] = Kr[2]) : Jr[Gr] += Wr, Yr += Bt(Wr);
          }
          for (var en in Jr)
            Ft(ze, en, Jr[en]);
        }
      ...
      function jt(ze) {
        ...
        for (var Qr = 0; Qr < ze.attributes.length; Qr++) {
          ...
          if (g(Jr, "hx-on") || g(Jr, "data-hx-on")) {
            var Gr = Jr.indexOf("-on") + 3, Yr = Jr.slice(Gr, Gr + 1);
            if (Yr === "-" || Yr === ":") {
              var Wr = Jr.slice(Gr + 1);
              g(Wr, ":") ? Wr = "htmx" + Wr : g(Wr, "-") ? Wr = "htmx:" + Wr.slice(1) : g(Wr, "htmx-") && (Wr = "htmx:" + Wr.slice(5)), Ft(ze, Wr, Zr);
            }
          }
        }
      }
      function _t(ze) {
        ...
        if (Qr.initHash !== Le(ze)) {
          ...
          Zr || (ne(ze, "hx-boost") === "true" ? lt(ze, Qr, Jr) : o(ze, "hx-trigger") && Jr.forEach(function(Wr) {
            Lt(ze, Wr, Qr, function() {
            });
      ...
      function ce(ze, Qr, Jr) {
        ...
        var Gr = ze.dispatchEvent(Zr), Yr = $t(Qr);
        if (Gr && Yr !== Qr) {
          var Wr = Wt(Yr, Zr.detail);
          Gr = Gr && ze.dispatchEvent(Wr);
        }
      ...
      function Kt(ze, Qr, Jr, Zr) {
        if (U()) {
          ...
          ze = B(ze);
          for (var Gr = E(localStorage.getItem("htmx-history-cache")) || [], Yr = 0; Yr < Gr.length; Yr++)
            if (Gr[Yr].url === ze) {
              Gr.splice(Yr, 1);
              break;
            }
          var Wr = { url: ze, content: Qr, title: Jr, scroll: Zr };
          for (ce(re().body, "htmx:historyItemCreated", { item: Wr, cache: Gr }), Gr.push(Wr); Gr.length > Q.config.historyCacheSize; )
            Gr.shift();
          for (; Gr.length > 0; )
            try {
      ...
      function ir(ze) {
        ...
        ce(re().body, "htmx:historyCacheMiss", Jr), Qr.open("GET", ze, !0), Qr.setRequestHeader("HX-Request", "true"), Qr.setRequestHeader("HX-History-Restore-Request", "true"), Qr.setRequestHeader("HX-Current-URL", re().location.href), Qr.onload = function() {
          if (this.status >= 200 && this.status < 400) {
            ...
            var Gr = Zt(), Yr = T(Gr), Wr = Ve(this.response);
            if (Wr) {
              var Kr = C("title");
              Kr ? Kr.innerHTML = Wr : window.document.title = Wr;
            }
            Ue(Gr, Zr, Yr), nr(Yr.tasks), Jt = ze, ce(re().body, "htmx:historyRestore", { path: ze, cacheMiss: !0, serverResponse: this.response });
          } else
      ...
      function hr(ze, Qr, Jr, Zr, Gr) {
        if (!(Zr == null || ur(ze, Zr))) {
          if (ze.push(Zr), fr(Zr)) {
            var Yr = ee(Zr, "name"), Wr = Zr.value;
            Zr.multiple && Zr.tagName === "SELECT" && (Wr = M(Zr.querySelectorAll("option:checked")).map(function(en) {
              return en.value;
            })), Zr.files && (Wr = M(Zr.files)), cr(Yr, Wr, Qr), Gr && vr(Zr, Jr);
          }
          if (h(Zr, "form")) {
            var Kr = Zr.elements;
            oe(Kr, function(en) {
      ...
      function dr(ze, Qr) {
        var Jr = [], Zr = {}, Gr = {}, Yr = [], Wr = ae(ze);
        Wr.lastButtonClicked && !se(Wr.lastButtonClicked) && (Wr.lastButtonClicked = null);
        var Kr = h(ze, "form") && ze.noValidate !== !0 || te(ze, "hx-validate") === "true";
        if (Wr.lastButtonClicked && (Kr = Kr && Wr.lastButtonClicked.formNoValidate !== !0), Qr !== "get" && hr(Jr, Gr, Yr, v(ze, "form"), Kr), hr(Jr, Zr, Yr, ze, Kr), Wr.lastButtonClicked || ze.tagName === "BUTTON" || ze.tagName === "INPUT" && ee(ze, "type") === "submit") {
          var en = Wr.lastButtonClicked || ze, tn = ee(en, "name");
          cr(tn, en.value, Gr);
        }
        ...
        }), Zr = le(Zr, Gr), { errors: Yr, values: Zr };
      }
      ...
      function wr(ze, Qr) {
        ...
        if (Q.config.scrollIntoViewOnBoost && ae(ze).boosted && !br(ze) && (Zr.show = "top"), Jr) {
          var Gr = D(Jr);
          if (Gr.length > 0)
            for (var Yr = 0; Yr < Gr.length; Yr++) {
              var Wr = Gr[Yr];
              if (Wr.indexOf("swap:") === 0)
                Zr.swapDelay = d(Wr.substr(5));
              else if (Wr.indexOf("settle:") === 0)
                Zr.settleDelay = d(Wr.substr(7));
              else if (Wr.indexOf("transition:") === 0)
                Zr.transition = Wr.substr(11) === "true";
              else if (Wr.indexOf("ignoreTitle:") === 0)
                Zr.ignoreTitle = Wr.substr(12) === "true";
              else if (Wr.indexOf("scroll:") === 0) {
                var Kr = Wr.substr(7), en = Kr.split(":"), tn = en.pop(), rn = en.length > 0 ? en.join(":") : null;
                Zr.scroll = tn, Zr.scrollTarget = rn;
              } else if (Wr.indexOf("show:") === 0) {
                var nn = Wr.substr(5), en = nn.split(":"), on = en.pop(), rn = en.length > 0 ? en.join(":") : null;
                Zr.show = on, Zr.showTarget = rn;
              } else if (Wr.indexOf("focus-scroll:") === 0) {
                var gn = Wr.substr(13);
                Zr.focusScroll = gn == "true";
              } else
                Yr == 0 ? Zr.swapStyle = Wr : b("Unknown modifier in hx-swap: " + Wr);
            }
        }
        return Zr;
      }
      ...
      function Cr(ze, Qr) {
        ...
        if (Qr.show) {
          ...
          if (Qr.showTarget) {
            ...
            Qr.showTarget === "window" && (Yr = "body"), Gr = ue(Jr, Yr);
          }
      ...
      function Rr(ze, Qr, Jr, Zr) {
        ...
        var Gr = te(ze, Qr);
        if (Gr) {
          var Yr = Gr.trim(), Wr = Jr;
          if (Yr === "unset")
            return null;
          Yr.indexOf("javascript:") === 0 ? (Yr = Yr.substr(11), Wr = !0) : Yr.indexOf("js:") === 0 && (Yr = Yr.substr(3), Wr = !0), Yr.indexOf("{") !== 0 && (Yr = "{" + Yr + "}");
          var Kr;
          Wr ? Kr = Tr(ze, function() {
      ...
      function he(ze, Qr, Jr, Zr, Gr, Yr) {
        ...
        if (Qr = cn.path, ze = cn.verb, sn = cn.headers, qn = cn.parameters, En = cn.errors, Qn = cn.useUrlParams, En && En.length > 0)
          return ce(Jr, "htmx:validation:halted", cn), ie(Wr), dn(), en;
      ...
      function Pr(ze, Qr) {
        ...
        var Yr = Qr.pathInfo.finalRequestPath, Wr = Qr.pathInfo.responsePath, Kr = ne(ze, "hx-push-url"), en = ne(ze, "hx-replace-url"), tn = ae(ze).boosted, rn = null, nn = null;
        return Kr ? (rn = "push", nn = Kr) : en ? (rn = "replace", nn = en) : tn && (rn = "push", nn = Wr || Yr), nn ? nn === "false" ? {} : (nn === "true" && (nn = Wr || Yr), Qr.pathInfo.anchor && nn.indexOf("#") === -1 && (nn = nn + "#" + Qr.pathInfo.anchor), { type: rn, path: nn }) : {};
      }
      function Mr(ze, Qr) {
        ...
        if (ce(ze, "htmx:beforeOnLoad", Qr)) {
          if (O(Jr, /HX-Trigger:/i) && _e(Jr, "HX-Trigger", ze), O(Jr, /HX-Location:/i)) {
            er();
            var Wr = Jr.getResponseHeader("HX-Location"), Kr;
            Wr.indexOf("{") === 0 && (Kr = E(Wr), Wr = Kr.path, delete Kr.path), Nr("GET", Wr, Kr).then(function() {
              tr(Wr);
            });
            return;
          }
